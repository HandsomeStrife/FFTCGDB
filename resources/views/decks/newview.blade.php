...
<div class="container">
    ...
    @elseif (isset($deck) && !empty($deck))
                ...
                <div class="panel panel-default">
                    <div class="card-list-section">
                        <div class="panel-body">
                            <div class="row">
                                <div class="col-md-4">
                                    <div class='title'><strong>Forwards @if (!empty($cards)) ({{ $cards->sum(function ($c) { if ($c->type == 'forward') { return $c->count; } }) }}) @else (0) @endif</strong></div>
                                    <ul class='card-list'>
                                ...
                                <div class="col-md-4">
                                    <div class='title'><strong>Backups @if (!empty($cards)) ({{ $cards->sum(function ($c) { if ($c->type == 'backup') { return $c->count; } }) }}) @else (0) @endif</strong></div>
                                    <ul class='card-list'>
                                        @forelse ($cards as $card)
                                            @if ($card->type == 'backup')
                                                @include('shared.listcarditem')
                                ...
                                <div class="col-md-4">
                                    <div class='title'><strong>Summons  @if (!empty($cards)) ({{ $cards->sum(function ($c) { if ($c->type == 'summon') { return $c->count; } }) }}) @else (0) @endif</strong></div>
                                    <ul class='card-list'>
                                        @forelse ($cards as $card)
                                            @if ($card->type == 'summon')
                                               @include('shared.listcarditem')
                        ...
                        <div class="panel-body">
                            <form method="post" action="{{ action('DeckController@addComment', ['deck_id' => $deck->id]) }}">
                                ...
                                <div class="form-group">
                                    <textarea id="add-comment-textarea" class="form-control" rows="3" name="comment" placeholder="Please enter your comment here. Type @ to mention users, and use square brackets to mention specific cards e.g. [25] or [1-025-C]"></textarea>
                                </div>
                    ...
                    @forelse ($deck->comments as $comment)
                        <div class="comment-item panel panel-default">
                            ...
                            <div class="panel-footer">
                                <p>{{ $comment->created_at->diffForHumans() }} by <a href="/u/{{ $comment->user->username }}">{{ $comment->user->name }}</a></p>
                            </div>
                        </div>
                    @empty
                        <div class="panel panel-default">
        ...
        <script type="text/javascript" src="https://cdnjs.cloudflare.com/ajax/libs/jquery.textcomplete/1.7.3/jquery.textcomplete.min.js"></script>
